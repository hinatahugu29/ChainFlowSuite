...
*   **v23.2 [Modular Refined]**:
    *   **Architecture Refactoring**: `FilePane` と `MainWindow` から重いロジックを抽出し、`core/actions.py` および `core/session.py` へ委譲。コードのモジュール性が向上し、メンテナンスが容易になりました。
    *   **Version Bump**: 全コンポーネントのバージョン情報を v23.2 へ統一。
*   **v23.1 [Fully Native]**:
    *   **Fully Native Core**: `chainflow_core` (Rust/PyO3) による高速なディレクトリ走査と、`NativeFileModel` による I/O ゼロ・レンダリングを導入。数万個のファイルも瞬時に表示可能。
    *   **Rename UX**: F2キーでのリネーム時、拡張子を除いたファイル名部分のみを自動選択する挙動を実装。
*   **v21.0 (Alpha)**:
