...
from PySide6.QtWidgets import (QFrame, QVBoxLayout, QWidget, QHBoxLayout, QLabel, 
                               ...
                               QSizePolicy, QApplication, QStyledItemDelegate, 
...
from models.native_model import NativeFileModel
from core import same_path, FileOperationWorker, logger
from core.actions import FileActions
...
class FilePane(QFrame):
    ...
    def __init__(self, title="Flow", parent_filer=None):
        ...
        # --- Model Architecture Change (V23.1 Full Native) ---
        # self.base_model = get_global_file_system_model()  # Removed in v23.1
    ...
    def eventFilter(self, watched, event):
        if event.type() == QEvent.Wheel:
            ...
            if event.key() in (Qt.Key_Return, Qt.Key_Enter):
                # 監視対象がView（QTreeView）またはそのViewportかつEnterが押された場合
                for view, proxy, _, _ in self.views:
                    if watched == view or (shiboken.isValid(view) and watched == view.viewport()):
    ...
    def _trigger_column_adjust(self):
        ...
        # v21.5 Fix: processEvents()はイベントループの再入を引き起こし、
        # タブ切替時の描画連鎖の原因となるため除去。
        # 50msタイマー遅延（BatchTreeView._resize_timer）の時点で
        # レイアウトは既に更新済みのため影響なし。
    ...
    def display_folders(self, paths):
        ...
        # 既存Viewのマップを作成 (path -> (view, model, sep, container))
        existing_map = {}
        for v, m, path, s in self.views:
            existing_map[path] = (v, m, s, v.parentWidget())
        ...
        for i, path in enumerate(self.current_paths):
            ...
            if path in existing_map:
                view, model, sep, container = existing_map[path]
                # splitter内の現在の位置を確認
                ...
                new_views_list.append((view, model, path, sep))
                ...
                # --- V23.1 Native Implementation ---
                model = NativeFileModel()
                model.sort_type = self.current_sort_col
                model.ascending = (self.sort_order == Qt.AscendingOrder)
                model.display_mode = self.display_mode
                model.show_hidden = self.show_hidden
                model.search_text = self.search_box.text()
                
                # v23.2 Fix: Ensure reference to FlowArea.marked_paths is linked correctly
                if self._marked_paths_ref is None and hasattr(self, 'parent_lane'):
                    if hasattr(self.parent_lane, 'parent_area'):
                         self._marked_paths_ref = self.parent_lane.parent_area.marked_paths

                model.marked_paths = self._marked_paths_ref if self._marked_paths_ref is not None else set()
                
                # Rust Scan & Initial Sort
                model.setRootPath(path)

                view = BatchTreeView(self)
                view.setModel(model)
                
                # Note: No setRootIndex needed because NativeFileModel is not a tree-filesystem-model
                ...
                view.setEditTriggers(QTreeView.EditKeyPressed | QTreeView.SelectedClicked)
                view.setHeaderHidden(False)
                view.setIndentation(0)
                
                # In V23.1, column indexes are: 0:Name, 1:Path, 2:Size, 3:Date
                view.hideColumn(1) 
                ...
                view.customContextMenuRequested.connect(lambda pos, v=view, m=model: self.open_context_menu(pos, v, m))
    ...
    def set_temp_highlight(self, path):
        ...
        for view, model, root_path, _ in self.views:
            if not shiboken.isValid(view):
                continue
    ...
    def _is_ancestor_of(self, ancestor_path, descendant_path):
        ...
        # Optimized: 文字列比較を優先。既に正規化されていることを期待する。
        # 呼び出し元の set_temp_highlight で target_path は normpath 済み。
        a = os.path.normcase(ancestor_path)
        d = os.path.normcase(descendant_path)
        
        return d.startswith(a + os.sep) or d == a
    ...
    def refresh_contents(self):
        ...
        for view, model, root_path, _ in self.views:
            ...
            for idx in view.selectionModel().selectedIndexes():
                if idx.column() == 0:
                    path = idx.data(Qt.UserRole) # Use UserRole directly
            ...
            if current_idx.isValid():
                current_path = current_idx.data(Qt.UserRole)
    ...
    def _restore_after_refresh(self, saved_states):
        ...
        for i, state in enumerate(saved_states):
            ...
            if state['current_path']:
                row = model.find_path_row(state['current_path'])
                if row >= 0:
                    idx = model.index(row, 0)
                    if idx.isValid():
        ...
        if view:
            ...
            for idx in selected_indexes:
                ...
                if path and os.path.exists(path):
                    ...
                    full_infos.append({"index": idx, "path": path, "is_dir": is_dir})
                    if not is_dir and path.lower().endswith('.zip'):
                        has_zip = True
        return {"paths": paths, "full_infos": full_infos, "has_zip": has_zip, "view": view, "model": proxy, "proxy": proxy}
    ...
    def action_paste(self, dest_dir=None):
        ...
        if cb.get("paths"):
            ...
            if cb["mode"] == "move":
                 # Clear internal clipboard after move (action_cut sets mode="move")
    ...
    def action_delete(self):
        FileActions.action_delete(self)

    def action_rename(self):
        FileActions.action_rename(self)

    def action_new_folder(self, view=None, model=None):
        FileActions.action_new_folder(self, view, model)

    def action_terminal(self, paths):
        FileActions.action_terminal(self, paths)

    def action_zip(self, selection):
        FileActions.action_zip(self, selection)

    def action_unzip(self, selection):
        FileActions.action_unzip(self, selection)
    ...
    def update_header_title(self):
        ...
        col_names = {0: "Name", 1: "Type", 2: "Size", 3: "Date"}
    ...
    def toggle_sort(self, col):
        ...
        for view, model, _, _ in self.views:
            if not shiboken.isValid(view):
                continue
            model.sort_type = col
            model.ascending = (self.sort_order == Qt.AscendingOrder)
    ...
    def on_item_clicked(self, index):
        ...
        if QApplication.keyboardModifiers() & Qt.AltModifier and self._marked_paths_ref is not None:
            # v23.2: Case-insensitive path normalization for matching
            p_norm = os.path.normcase(os.path.abspath(path))
            if p_norm in self._marked_paths_ref:
                self._marked_paths_ref.remove(p_norm)
            else:
                self._marked_paths_ref.add(p_norm)
    ...
    def on_double_clicked(self, index, view):
        ...
        if path:
            ...
            if os.path.isdir(path):
                ...
                # ファイルならOS標準のアプリで開く
                FileActions.execute_path(self, path)
        ...
        if hasattr(self, 'parent_lane') and hasattr(self.parent_lane, 'parent_area'):
            ...
            for lane in area.lanes:
                for pane in lane.panes:
                    for v, p, _, _ in pane.views:
                        if not shiboken.isValid(v):
                            continue
                        # v23.2: Sync mark reference to model AND view delegate
                        if hasattr(p, 'marked_paths'):
                            p.marked_paths = pane._marked_paths_ref if pane._marked_paths_ref is not None else set()
                        
                        if hasattr(v, 'update_marked_paths'):
                            v.update_marked_paths(pane._marked_paths_ref if pane._marked_paths_ref is not None else set())
                            
                        # キャッシュ更新
                        if hasattr(p, 'updateMarkedCache'):
                            p.updateMarkedCache()

                        # v22.1 Performance: BackgroundRoleのみの通知で描画を促す
                        top_left = p.index(0, 0)
                        bottom_right = p.index(p.rowCount() - 1, p.columnCount() - 1)
                        if top_left.isValid() and bottom_right.isValid():
                            p.dataChanged.emit(top_left, bottom_right, [Qt.BackgroundRole])

    def action_convert_to_pdf(self, paths):
        FileActions.action_convert_to_pdf(self, paths)

    def action_new_file(self, view=None, proxy=None):
        FileActions.action_new_file(self, view, proxy)

    def action_new_folder(self, view=None, proxy=None):
        FileActions.action_new_folder(self, view, proxy)
    ...
    def navigate_to(self, view, path):
        ...
        for i, info in enumerate(self.views):
            v, model, p = info[0], info[1], info[2]
            if v == view:
                # ターゲットルート更新
                model.setRootPath(path)
                
                # タプルを更新 (view, model, path, sep)
                new_info = list(info)
                new_info[2] = path
