...
class PluginManager:
    ...
    def _detect_root_dir(self):
        """Detect root directory (Dev or Frozen). V23.1: Support Internal folder."""
        # Nuitka/PyInstaller standalone detection
        is_standalone = getattr(sys, 'frozen', False) or "__compiled__" in globals() or "main.exe" in sys.executable.lower()
        
        if is_standalone:
            # standalone: .../Internal/main.exe
            base = os.path.dirname(sys.executable)
            if os.path.basename(base).lower() == "internal":
                return os.path.dirname(base) # Up to Suite Root
            return base
        else:
            # Dev: G:/CODE/.../ChainFlowFiler_v23_1/ (script is in core/)
            # This points to the app root (e.g. ChainFlowFiler_v23_1/)
