...
class PreviewPane(QWidget):
    ...
    def __init__(self):
        ...
        self.md = (MarkdownIt('commonmark', {'breaks': True, 'html': True})
                   ...
                   .use(texmath_plugin, delimiters='dollars').use(anchors_plugin).enable('table'))
    ...
    def setup_ui(self):
        ...
        header_container.setStyleSheet("background-color: #2D2D30;")
        header_container.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Fixed)
        h_layout = QHBoxLayout(header_container)
        h_layout.setContentsMargins(15, 5, 10, 5)

        header_label = QLabel("LIVE PREVIEW")
        header_label.setStyleSheet("color: #8F8F8F; font-weight: bold; font-size: 11px; border:none;")
        h_layout.addWidget(header_label)
        h_layout.addStretch()

        self.refresh_btn = QPushButton("Force Refresh")
        self.refresh_btn.setToolTip("レイアウト計算を強制的にやり直し、空白ページを解消します")
        self.refresh_btn.setStyleSheet("""
            QPushButton {
                background-color: #3E3E42; color: #CCC; border: none; 
        ...
        <head>
            ...
            <script>
                ...
                window.updateOverlays = function() {
                    ...
                    const elements = Array.from(paper.children);
    ...
    def force_refresh(self):
        if self._is_initialized:
            self.web_view.page().runJavaScript("window.forceRefresh();")
