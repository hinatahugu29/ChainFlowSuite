...
class MarkdownHighlighter(QSyntaxHighlighter):
    ...
    def __init__(self, parent=None):
        ...
        color_tag = QColor("#4B4B4B")    # Dimmer (Dark Gray) for HTML noise
        ...
        # 9. IDE-Grade HTML タグ多色ハイライト
        # ベース（<, >, = 等）は今のDimmer色で背景に沈める
        html_base_format = QTextCharFormat()
        html_base_format.setForeground(color_tag) # #4B4B4B

        # タグ名（div, span等）はピンク（IDE標準準拠）
        html_tagname_format = QTextCharFormat()
        html_tagname_format.setForeground(QColor("#C586C0"))

        # 属性名（class, style等）は水色
        html_attr_format = QTextCharFormat()
        html_attr_format.setForeground(QColor("#9CDCFE"))

        # 属性値（"..."の中身）はオレンジ
        html_string_format = QTextCharFormat()
        html_string_format.setForeground(QColor("#CE9178"))

        # QRegularExpressionは上に書かれたものから順に適用して上書きする
        # (1) まずタグのブロック全体をDimmer（暗色）で塗りつぶす
        self.highlighting_rules.append((QRegularExpression(r"<[^>]+>"), html_base_format))
        # (2) タグ名（< または </ の直後の文字）をピンクで上塗り
        self.highlighting_rules.append((QRegularExpression(r"(?<=<)[a-zA-Z0-9\-]+|(?<=</)[a-zA-Z0-9\-]+"), html_tagname_format))
        # (3) 属性名（= の直前の文字）を水色で上塗り
        self.highlighting_rules.append((QRegularExpression(r"\b[a-zA-Z\-]+(?=\=)"), html_attr_format))
        # (4) 属性値の文字列（"" または ''）をオレンジで上塗り
        self.highlighting_rules.append((QRegularExpression(r"\"[^\"]*\""), html_string_format))
        self.highlighting_rules.append((QRegularExpression(r"'[^']*'"), html_string_format))
...
class EditorPane(QWidget):
    ...
    def setup_ui(self):
        ...
        self.editor.customContextMenuRequested.connect(lambda pos: self._show_context_menu(pos, self.editor))

        self.editor_sub = MarkdownEditor()
        self.editor_sub.setStyleSheet(self.editor.styleSheet())
        self.editor_sub.setLineWrapMode(QPlainTextEdit.WidgetWidth)
        self.editor_sub.setContextMenuPolicy(Qt.CustomContextMenu)
        self.editor_sub.customContextMenuRequested.connect(lambda pos: self._show_context_menu(pos, self.editor_sub))
        
        # Share the same document! This is the magic for split view.
        self.editor_sub.setDocument(self.editor.document())
        
        # Add to splitter
        self.editor_splitter.addWidget(self.editor)
        self.editor_splitter.addWidget(self.editor_sub)
        
        # Initial pane sizes (e.g. 70% top, 30% bottom)
        self.editor_splitter.setSizes([700, 300])

        # Add toggle split button to toolbar
        ...
        self.btn_split.toggled.connect(self._toggle_split_view)
    ...
    def _insert_tasklist(self):
        task_md = "\n- [ ] Task 1\n- [ ] Task 2\n- [x] Completed task\n\n"
        editor = self._active_editor()
        editor.insertPlainText(task_md)
        editor.setFocus()
    ...
    def _setup_additional_shortcuts(self):
        ...
        act_bold.setShortcutContext(Qt.WindowShortcut)
        ...
        act_italic.setShortcutContext(Qt.WindowShortcut)
        ...
        act_stamp.setShortcutContext(Qt.WindowShortcut)
        act_stamp.triggered.connect(lambda: self._wrap_selection("stamp"))
        self.addAction(act_stamp)
        ...
        act_var.setShortcutContext(Qt.WindowShortcut)
        act_var.triggered.connect(lambda: self._apply_inline_format("{{", "}}"))
        self.addAction(act_var)
        
        # Switch Editor Focus: Ctrl+Tab
        act_switch = QAction("Switch Focus", self)
        act_switch.setShortcut(QKeySequence("Ctrl+Tab"))
        act_switch.setShortcutContext(Qt.WindowShortcut)
        act_switch.triggered.connect(self._switch_editor_focus)
        self.addAction(act_switch)

    def _switch_editor_focus(self):
        if self.editor.hasFocus() and self.editor_sub.isVisible():
            self.editor_sub.setFocus()
        else:
            self.editor.setFocus()
    ...
    def _show_context_menu(self, position, editor=None):
        ...
        if cursor.hasSelection():
            ...
            # Use lambda to capture the specific editor for this context menu
            action_save_snippet.triggered.connect(lambda: self._emit_save_snippet(editor))
            
        menu.exec(editor.viewport().mapToGlobal(position))

    def _emit_save_snippet(self, editor=None):
        if editor is None:
            editor = self._active_editor()
        cursor = editor.textCursor()
        if cursor.hasSelection():
