...
class FilePane(QFrame):
    ...
    def display_folders(self, paths):
        ...
        for i, path in enumerate(self.current_paths):
            # v22.1 Performance: os.path.existsはネットワーク上で遅いため、モデルベースの判定に変更
            model_idx = self.base_model.index(path)
            if not model_idx.isValid(): continue
    ...
    def refresh_contents(self):
        ...
        # v22.1 Performance: setRootPathは重いため省略し、proxy.invalidateのみで再フィルタ
        # (ゴースト対策はinvalidateで十分カバー。問題が再発した場合はsetRootPathを復活させる)
        for view, proxy, root_path, _ in self.views:
            proxy.invalidate()
                ...
                for pane in lane.panes:
                    for _, p, _, _ in pane.views:
                        ...
                        if hasattr(p, 'updateMarkedCache'):
                            p.updateMarkedCache()
                        # v22.1 Performance: layoutChanged → dataChanged に変更
                        # layoutChangedはView全体を再構築するため重い。BackgroundRoleのみの通知で十分。
