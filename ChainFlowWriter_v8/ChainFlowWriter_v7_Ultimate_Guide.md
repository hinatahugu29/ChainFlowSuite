...
## 4. v8 の主要な進化点：Split View (上下分割エディタ)

ChainFlow Writer v8 では、エディタ画面を上下に分割し、同一ドキュメントの異なる箇所を同時に表示・編集できる **Split View機能** を独自に搭載しました。

- **「見ながら書く」没入感**: 上段のエディタに `<style>` や全体の構成要素を表示させたまま、下段で Markdown 本文に集中して書き進めることができます。
- **完全なリアルタイム同期**: どちらのエディタで入力しても、即座にもう一方と右側のプレビューに反映されます。もちろん保存も一括で行われます。
- **フォーカス連動ツールバー**: 「Bold」や「Insert Table」などのツールバーボタンや各種ショートカットは、現在カーソルが置かれている（フォーカスが当たっている）エディタに対して正確に機能します。
- **表示幅の自由な変更**: 境界線をドラッグして表示比率を変更したり、ツールバーの「Split View」ボタンで下段エディタを一時的に隠すことも可能です。
