...
from PySide6.QtCore import Qt, QSize, QPropertyAnimation, QEasingCurve, QPoint, QTimer, QUrl
...
class QuickLookWindow(QWidget):
    ...
    def setup_ui(self):
        ...
        if HAS_WEBENGINE:
            try:
                ...
                # v17.5 Dark background to prevent white flash
                self.web_view.setStyleSheet("background: #1e1e1e;") 
                self.web_view.hide()
                self.content_layout.addWidget(self.web_view)
                
                # v17.3 Fix: Install EventFilter to capture Wheel Events on WebEngine
                self.web_view.installEventFilter(self)
                for child in self.web_view.children():
                    child.installEventFilter(self)
                
                # v17.6 Fix: Connect signal (Missing connection caused invisible HTML)
                self.web_view.loadFinished.connect(self._on_web_load_finished)
                
                # v17.7 Warm-up: Pre-initialize Chromium subprocess to prevent GUI flash on first use
                self.web_view.load(QUrl("about:blank"))
                    
            except Exception as e:
                self.log(f"WebEngine Init Error: {e}")
    ...
    def show_file(self, path, edit_mode=False):
        ...
        try:
            ...
            # v17.0 HTML Support
            if ext in ['.html', '.htm'] and HAS_WEBENGINE and self.web_view:
                # QUrl.fromLocalFile を使わないと正しくロードされないことがある
                url = QUrl.fromLocalFile(path)
                
                self.web_view.load(url)
                self.pdf_btn.show() 
                
                # v17.4: Focus logic is triggered by load finished signal
                return
    ...
    def export_pdf(self):
        """v17.0 Displayed HTML to PDF"""
        if not HAS_WEBENGINE or not self.web_view or not self.web_view.isVisible():
            return
            
        # デフォルトファイル名
        base_name = os.path.splitext(os.path.basename(self._current_path))[0]
        default_path = os.path.join(os.path.dirname(self._current_path), f"{base_name}.pdf")
        ...
        def pdf_finished(file_path, success):
            ...
            # cleanup connection if needed, though lambda captures context. 
            # In real generic cleanup, might want to disconnect.
            try:
        ...
        try:
            # Connect signal
            # Note: PySide6.QtWebEngineCore.QWebEnginePage.pdfPrintingFinished (filePath, success)
            self.web_view.page().pdfPrintingFinished.connect(pdf_finished)
            
            # Trigger
            self.web_view.page().printToPdf(save_path)
        except Exception as e:
             QMessageBox.critical(self, "Error", f"PDF Export Error:\n{e}")
    ...
    def wheelEvent(self, event):
        # Parent level hook (fallback)
        if hasattr(self, 'web_view') and self.web_view and self.web_view.isVisible():
            delta = event.angleDelta().y()
            scroll_y = -delta 
            self.web_view.page().runJavaScript(f"window.scrollBy(0, {scroll_y});")
        super().wheelEvent(event)

    def eventFilter(self, obj, event):
        # v17.3 Fix: Capture wheel events directly from WebEngine children
        from PySide6.QtCore import QEvent
        if event.type() == QEvent.Wheel:
             # Check if it's related to web view
             if hasattr(self, 'web_view') and self.web_view and self.web_view.isVisible():
                 # We simply force scroll and consume the event to prevent native issues
                 delta = event.angleDelta().y()
                 scroll_y = -delta
                 self.web_view.page().runJavaScript(f"window.scrollBy(0, {scroll_y});")
                 return True
    ...
    def _on_web_load_finished(self, ok):
        # Skip processing for warm-up blank page
        if self.web_view and self.web_view.url().toString() == "about:blank":
            return
            
        if not ok:
            self.log("WebEngine Load Failed")
            if self.web_view:
                self.web_view.hide()
                self.show_info(f"Failed to load web content.\nURL: {self.web_view.url().toString()}")
        else:
             # v17.8: Inject Custom Scrollbar CSS
             self._inject_scrollbar_css()
             QTimer.singleShot(100, self._ensure_web_focus)

    def _inject_scrollbar_css(self):
        """v17.8 Inject CSS to style WebEngine scrollbar like the app theme"""
        ...
        """
        # Minify logic slightly for JS string safety
        css = css.replace('\n', ' ').replace('  ', '')
    ...
    def _ensure_web_focus(self):
        """v17.4 Helper to ensure focus and event hooks are applied"""
        if hasattr(self, 'web_view') and self.web_view:
            # v17.5: Show view only after load/focus setup to prevent flash
            self.web_view.show()
            # self.activateWindow() # v17.6 Remove to prevent main window flash
            self.web_view.setFocus()
            
            # Re-hook children (render widget takes time to appear)
            # Focus Proxy
