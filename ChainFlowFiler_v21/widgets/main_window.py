...
class ChainFlowFiler(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("ChainFlow Filer v21")
    ...
    def load_session(self):
        ...
        try:
            ...
            for i, t_data in enumerate(tabs_data):
                ...
                # v19.1: セッション復元後、動的タイトルに上書き
                dynamic_name = area.get_representative_name()
                self.tab_widget.setTabText(idx, dynamic_name)
    ...
    def highlight_context(self):
        ...
        # フォーカスを持っているViewを優先
        for view, proxy, root_path, _ in active_pane.views:
            try:
                if view.hasFocus():
                    idx = view.currentIndex()
                    if idx.isValid():
                        src_idx = proxy.mapToSource(idx)
                        target_path = active_pane.base_model.filePath(src_idx)
                    else:
                        # v14.3 Fix: 項目がない（空フォルダ）場合でも、そのView自体がアクティブなら
                        # そのフォルダパス自体をターゲットとする
                        target_path = root_path
                    break
            except RuntimeError:
                continue # 既に削除されたオブジェクトはスキップ
        
        # フォールバック: currentIndexが有効な最初のView
        if not target_path:
            for view, proxy, _, _ in active_pane.views:
                try:
                    idx = view.currentIndex()
                    if idx.isValid():
                        src_idx = proxy.mapToSource(idx)
